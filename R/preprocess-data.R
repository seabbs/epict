process_data <- function(dt_raw) {
  ...
  out <- merge(
    out, first_pos_test_date_dt,
    by = c("id", "infection_id")
  )

  out[,
    time_since_first_pos := as.numeric(
      swab_date - first_pos_test_date,
      units = "days"
    ),
    by = c("id", "infection_id")
  ]
  ...
  out[
    !symptoms %in% c("0", "1", "unknown"),
    symptoms := ifelse(!is.na(symptom_onset_date), "1", "unknown")
  ]
  # Make variables factors
  facs <- c(
    "no_vaccines", "VOC", "swab_type", "dose_1", "dose_2", "dose_3",
